    ...
    return (
      <div className="relative w-20 h-20">
        <svg className="w-full h-full" viewBox="0 0 80 80">
          ...
          />
          <circle
            className={`transition-all duration-1000 ease-out ${getScoreStrokeColor(score)}`}
            strokeWidth="8"
...
const AnalysisCard: React.FC<{ analysis: CandidateAnalysis; isTopPick: boolean }> = ({ analysis, isTopPick }) => (
    <div className={`bg-white rounded-lg border p-6 transition-all duration-300 ${isTopPick ? 'border-green-500' : 'border-[#e7edf4]'}`}>
      <div className="flex flex-col sm:flex-row gap-6">
        <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
           <h3 className="text-lg font-bold text-[#0d141c]">{analysis.candidateName}</h3>
            {isTopPick && (
                <div className="mt-2 flex items-center gap-1.5 bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-semibold">
                    <StarIcon/> Top Pick
                </div>
            )}
           <div className="mt-4">
        ...
        <div className="flex-1">
          <p className="text-[#49739c] italic">"{analysis.summary}"</p>
              ...
              <ul className="space-y-1.5">
                {analysis.pros.map((pro, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <CheckIcon /> <span className="text-sm text-[#0d141c]">{pro}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul className="space-y-1.5">
                {analysis.cons.map((con, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <CrossIcon /> <span className="text-sm text-[#0d141c]">{con}</span>
                  </li>
                ))}
              </ul>
            </div>
...
export const ResultsDisplay: React.FC<{ analysis: AnalysisResult }> = ({ analysis }) => {
  ...
  return (
    <div className="space-y-6 mt-8">
       <h2 className="text-[#0d141c] tracking-light text-[32px] font-bold leading-tight text-center mb-2">Analysis Results</h2>
       <p className="text-center text-[#49739c] max-w-2xl mx-auto text-base font-normal leading-normal">
